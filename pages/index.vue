...
export default {
  ...
  data: () => ({
    map: null,
    pledgePins: new Array(3),
    protectPins: new Array(3)
  }),
  methods: {
    ...
    GeneratePins() {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        var lng = (Math.random() * lngMagnitude * 0.8) + mapBounds.getWest() + lngMagnitude * 0.1;
    ...
    CreatePledgePin(pinId,  lat, lng) {
      ...
      if (this.pledgePins[pinId] != null) {
        this.map.removeLayer(this.pledgePins[pinId]);
      }
      this.pledgePins[pinId] = marker;
    }
  },
  mounted() {
